...
import com.technozi.control.VaccineCertificateController;
import com.technozi.enumerated.UserType;

public class VaccineCertUI extends HttpServlet {
			...
			for (Cookie cookie : cookies) {
				if (cookie.getValue().equals(session.getAttribute("username"))) {
					...
					if (userType.equals(UserType.Public.toString())) {
						VaccineCertificateController vcc = new VaccineCertificateController();
						request.setAttribute("vaccineRecord",
								vcc.requestVaccineCertificate(session.getAttribute("username").toString()));
						System.out.println("Directing to VaccineCertUI..");
						request.getRequestDispatcher("VaccineCert.jsp").forward(request, response);
					} else {
